...
This ROS 2 package contains the simulation and control nodes for an autonomous robot searching for ArUco markers in an enviroment. Developed for the Experimental Robotics Lab class, it allows you to run missions with either a 2-wheel or 4-wheel robot in Gazebo, using computer vision to detect targets and navigate the environment.
...
The mission logic is implemented through three dedicated ROS 2 nodes, all located in the `erl_assignment_2/` folder. They represents the actions implemented in the PDDL code in the folder `pddl/`
...
* launching all the nodes of plansys2 that we need.
